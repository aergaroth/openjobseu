...
def upgrade() -> None:
    ...
    conn.execute(
        sa.text("CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_jobs_title_trgm ON jobs USING GIN (title gin_trgm_ops);")
    )
    conn.execute(
        sa.text(
            "CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_jobs_company_name_trgm ON jobs USING GIN (company_name gin_trgm_ops);"
        )
    )
    conn.execute(
        sa.text(
            "CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_jobs_remote_scope_trgm ON jobs USING GIN (remote_scope gin_trgm_ops);"
        )
    )
    ...
    conn.execute(
        sa.text(
            "CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_companies_legal_name_trgm ON companies USING GIN (legal_name gin_trgm_ops);"
        )
    )
    conn.execute(
        sa.text(
            "CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_companies_brand_name_trgm ON companies USING GIN (brand_name gin_trgm_ops);"
        )
    )
