...
data "google_service_account" "cloud_run_runtime" {
  account_id = "cloudrun-prod-runtime"
}
...
resource "google_iam_workload_identity_pool_provider" "github_actions" {
  ...
  description                        = "Accept GitHub Actions OIDC tokens for deploy, Terraform plan, and asset publication in ${local.github_repository_full_name}."

  attribute_condition = join(" && ", [
    "assertion.repository == '${local.github_repository_full_name}'",
    "assertion.repository_owner == '${var.github_repository_owner}'",
    "((assertion.event_name in ['push', 'workflow_dispatch']) && assertion.ref == 'refs/heads/main') || (assertion.event_name == 'pull_request' && assertion.base_ref == 'main')",
  ])
...
resource "google_service_account" "github_deploy" {
  ...
  description  = "Builds the container image and applies Terraform for the prod environment."
}
...
resource "google_service_account" "github_terraform_plan" {
  ...
  description  = "Reads Terraform state and infrastructure metadata for prod pull request plans."
}

resource "google_service_account" "github_assets_publish" {
  account_id   = "github-assets-publish"
  display_name = "GitHub Actions assets publish"
  description  = "Publishes frontend/index.html, frontend/style.css, and frontend/feed.js to the public bucket after a prod release."
}
...
resource "google_service_account_iam_member" "github_assets_wif_user" {
  service_account_id = google_service_account.github_assets_publish.name
  role               = "roles/iam.workloadIdentityUser"
  member             = "principal://iam.googleapis.com/${google_iam_workload_identity_pool.github_actions.name}/subject/${local.github_ref_subject}"
}
...
resource "google_storage_bucket_iam_member" "github_assets_bucket_object_admin" {
  bucket = var.public_feed_bucket
  role   = "roles/storage.objectAdmin"
  member = "serviceAccount:${google_service_account.github_assets_publish.email}"
}
...
output "github_actions_workload_identity_provider" {
  description = "Full Workload Identity Provider resource name for GitHub Actions in prod."
  value       = google_iam_workload_identity_pool_provider.github_actions.name
}

output "github_actions_deploy_service_account" {
  description = "Service account email for prod build + deploy workflow impersonation."
  value       = google_service_account.github_deploy.email
}

output "github_actions_terraform_plan_service_account" {
  description = "Service account email for prod Terraform plan workflow impersonation."
  value       = google_service_account.github_terraform_plan.email
}

output "github_actions_assets_publish_service_account" {
  description = "Service account email for prod frontend asset publication impersonation."
  value       = google_service_account.github_assets_publish.email
}
