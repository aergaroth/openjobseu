...
def test_smartrecruiters_fetch_detail_fallback(monkeypatch):
    ...
    def mock_get(url, *args, **kwargs):
        ...
        resp = MagicMock()
        resp.json.return_value = {"jobAd": {"sections": {"jobDescription": {"text": "Full Description"}}}}
        return resp

    # Mock the get to return lists on first call, details on later calls
    monkeypatch.setattr(
        adapter.session,
        "get",
        lambda url, *a, **kw: mock_post_resp if "?limit" in url else mock_get(url),
    )
...
def test_smartrecruiters_probe_jobs_success(monkeypatch):
    ...
    mock_resp.json.return_value = {
        "content": [
            {
                "name": "Remote Dev",
                "location": {"city": "Remote", "country": "PL", "remote": True},
                "releasedDate": "2023-01-01T00:00:00Z",
            },
            {"name": "Local Dev"},
        ]
    }
