...
def test_extract_candidates_teamtailor_subdomain():
    cands = _extract_candidates_from_text("https://career.teamtailor.com/jobs", "final_url")
    assert any(c.provider == "teamtailor" and c.slug == "career" for c in cands)


def test_score_requires_final_or_multiple_occurrences():
