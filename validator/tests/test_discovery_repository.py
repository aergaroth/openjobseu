...
def test_insert_source_company():
    with engine.begin() as conn:
        ...
        row = conn.execute(text("SELECT brand_name, careers_url FROM companies WHERE legal_name = 'Acme Corp'")).mappings().one()
...
def test_load_and_update_discovery_companies():
    with engine.begin() as conn:
        ...
        conn.execute(text("UPDATE companies SET careers_url = 'http://foo' WHERE company_id = :id"), {"id": comp2_id})
        conn.execute(text("UPDATE companies SET careers_url = 'http://bar', ats_provider = 'lever' WHERE company_id = :id"), {"id": comp3_id})
        ...
        row = conn.execute(text("SELECT careers_last_checked_at FROM companies WHERE company_id = :id"), {"id": comp2_id}).mappings().one()
