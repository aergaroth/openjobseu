...
def test_job_processing_handles_approved_and_rejected(monkeypatch):
    # Mock dependencies
    monkeypatch.setattr(
        "app.domain.jobs.job_processing.apply_policy",
    ...
    monkeypatch.setattr(
        "app.domain.jobs.job_processing.detect_salary_transparency",
        lambda desc, detected: "transparent",
    )
...
def test_job_processing_early_exit_on_none_job(monkeypatch):
    monkeypatch.setattr(
        "app.domain.jobs.job_processing.apply_policy",
        lambda job, source: (None, "hard_reject"),
    )
...
def test_job_processing_salary_parsing_low_confidence(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.domain.jobs.job_processing.detect_salary_transparency",
        lambda desc, detected: "disclosed",
    )
    monkeypatch.setattr("app.domain.jobs.job_processing.compute_job_quality_score", lambda job: 50)
