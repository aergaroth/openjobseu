...
def test_salary_parser_cases(case):
    ...
    if case.get("min") is None:
        ...
        assert extracted["salary_min"] == case["min"], (
            f"Min mismatch for {case_label}: Expected {case['min']}, got {extracted['salary_min']}"
        )
        assert extracted["salary_max"] == case["max"], (
            f"Max mismatch for {case_label}: Expected {case['max']}, got {extracted['salary_max']}"
        )
        assert extracted["salary_currency"] == case["currency"], (
            f"Currency mismatch for {case_label}: Expected {case['currency']}, got {extracted['salary_currency']}"
        )
        assert extracted["salary_period"] == case["period"], (
            f"Period mismatch for {case_label}: Expected {case['period']}, got {extracted['salary_period']}"
        )
        assert extracted["salary_confidence"] >= case["confidence"], (
            f"Confidence too low for {case_label}: Expected >= {case['confidence']}, got {extracted['salary_confidence']}"
        )
