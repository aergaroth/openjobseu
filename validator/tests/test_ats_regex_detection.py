...
def test_detect_greenhouse_slug():
    assert _detect_provider("https://boards.greenhouse.io/acme-team") == (
        "greenhouse",
        "acme-team",
    )
...
def test_detect_workable_slug():
    assert _detect_provider("https://apply.workable.com/acme-io") == (
        "workable",
        "acme-io",
    )
...
def test_candidate_links_limited_to_max():
    anchor_html = "".join(f"<a href='/jobs/{i}'>Job {i}</a>" for i in range(MAX_SECONDARY_LINKS + 3))
