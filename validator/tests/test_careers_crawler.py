...
def test_fetch_careers_page_exception(monkeypatch):
    import requests
    monkeypatch.setattr(crawler_module.requests, "get", MagicMock(side_effect=requests.RequestException("Fail")))
...
def test_detect_provider():
    assert crawler_module._detect_provider("https://boards.greenhouse.io/acme") == ("greenhouse", "acme")
    assert crawler_module._detect_provider("https://boards.greenhouse.io/embed/job_board?for=acme") == ("greenhouse", "acme")
    assert crawler_module._detect_provider("https://jobs.lever.co/lever-slug") == ("lever", "lever-slug")
...
def test_detect_provider_from_redirects():
    ...
    assert crawler_module._detect_provider_from_redirects(mock_resp) == ("greenhouse", "redirect-slug")
...
def test_run_careers_discovery_happy_path(monkeypatch):
    ...
    monkeypatch.setattr(crawler_module, "_detect_provider_from_redirects", lambda r: ("lever", "acme-corp"))
    monkeypatch.setattr(crawler_module, "probe_ats", lambda p, s: {"jobs_total": 2, "remote_hits": 1, "recent_job_at": "2026-03-01T00:00:00Z"})
