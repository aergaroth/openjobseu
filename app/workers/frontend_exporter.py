...
from app.api.jobs import serialize_feed_job, FEED_LIMIT, FEED_VERSION, FEED_MIN_COMPLIANCE_SCORE
...
def run_frontend_export() -> dict:
    ...
    try:
        ...
        feed_blob.upload_from_string(json.dumps(payload, ensure_ascii=False), content_type="application/json")
        uploaded_files += 1
        
        logger.info("frontend_exported_to_gcs", extra={"job_count": len(jobs), "uploaded_files": uploaded_files})
        return {"status": "ok", "exported_jobs": len(jobs), "uploaded_files": uploaded_files}
