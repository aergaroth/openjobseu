...
def log_ingestion(
    ...
    **extra_fields,
    ...
    if phase == "ingestion_summary":
        ...
        payload.update(
            {
                RemoteClass.REMOTE_ONLY.value: remote_model_counts.get(RemoteClass.REMOTE_ONLY.value, 0),
                "remote_non_remote": remote_model_counts.get(RemoteClass.NON_REMOTE.value, 0),
                "remote_geo_restricted": remote_model_counts.get("remote_but_geo_restricted", 0),
                f"remote_{RemoteClass.UNKNOWN.value}": remote_model_counts.get(RemoteClass.UNKNOWN.value, 0),
            }
        )
