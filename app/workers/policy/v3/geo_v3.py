...
def classify_geo_v3(
    *,
    ...
    for kw in NON_EU_REGION_CUSTOM:
        if kw in full_text:
            return {"geo_class": "non_eu", "reason": kw}
    ...
    for kw in EU_REGION_CUSTOM:
        if kw in full_text:
            return {"geo_class": "eu_region", "reason": kw}
    ...
    if scope_l:
        ...
        if found_eu and found_non_eu:
            return {"geo_class": "eu_region", "reason": "mixed_region"}

        if found_eu:
            return {"geo_class": "eu_member_state", "reason": "explicit_country"}

        if found_non_eu:
            return {"geo_class": "non_eu", "reason": "explicit_country"}
    ...
    for country in EU_MEMBER_STATES:
        if country in full_text:
            return {"geo_class": "eu_member_state", "reason": country}
    ...
    for kw in UK_KEYWORDS:
        if kw in full_text:
            return {"geo_class": "uk", "reason": kw}

    # ------------------------------------------------------------
    # 6 Unknown
    # ------------------------------------------------------------
    return {"geo_class": "unknown", "reason": None}